...
![Strife bot icon](assets/bot-icon.png)

Strife is one node in the 13-bot Discord music fleet. Named after Cloud Strife — its Discord display name is simply "Cloud". It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Cloud)**
- Internal node name: `strife`
- Command namespace: `/strife_main_*`
...
All commands are namespaced as `/strife_main_<command>`.
