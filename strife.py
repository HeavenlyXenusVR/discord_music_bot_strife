    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title FROM strife_queue WHERE guild_id = %s AND bot_name = 'strife' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows:
                    await cur.execute("DELETE FROM strife_queue WHERE id = %s AND guild_id = %s AND bot_name = 'strife'", (r[0], interaction.guild.id))
                    await cur.execute(
                        "DELETE FROM strife_queue_backup WHERE video_url = %s AND title = %s AND guild_id = %s AND bot_name = 'strife' LIMIT 1",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for url, title in q:
                    await cur.execute("INSERT INTO strife_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)", (interaction.user.id, name, url, title))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                res = await cur.fetchone()
                if res and res[0] != 'none': return await interaction.response.send_message(embed=discord.Embed(description="❌ **Conflict:** Disable standard Filters via `/strife_main_filter none` first.", color=discord.Color.red()), ephemeral=True)
