    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                try: await cur.execute("ALTER TABLE strife_playback_state ADD COLUMN title TEXT")
                except: pass
                try: await cur.execute("ALTER TABLE strife_playback_state ADD COLUMN is_paused BOOLEAN DEFAULT FALSE")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE strife_guild_settings MODIFY loop_mode VARCHAR(10) DEFAULT 'queue'")
                except: pass
                try: await cur.execute("ALTER TABLE strife_guild_settings ADD COLUMN fade_seconds FLOAT DEFAULT 5.0")
                except: pass
                try: await cur.execute("ALTER TABLE strife_guild_settings ADD COLUMN fade_curve VARCHAR(20) DEFAULT 'linear'")
                except: pass
                try: await cur.execute("UPDATE strife_guild_settings SET loop_mode = 'queue' WHERE loop_mode IS NULL OR loop_mode NOT IN ('off', 'song', 'queue')")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE strife_queue ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE strife_queue_backup ADD COLUMN bot_name VARCHAR(50)")
                except: pass
                try: await cur.execute("ALTER TABLE strife_queue ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE strife_queue_backup ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                try: await cur.execute("ALTER TABLE strife_history ADD COLUMN requester_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX strife_track_intelligence_recent_idx ON strife_track_intelligence (guild_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_track_intelligence_requester_idx ON strife_track_intelligence (guild_id, last_requester_id, last_played)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_user_affinity_recent_idx ON strife_user_track_affinity (guild_id, user_id, last_requested)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_smart_recommendations_recent_idx ON strife_smart_recommendations (guild_id, created_at)")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE strife_voice_state ADD COLUMN reconnect_attempts INT NOT NULL DEFAULT 0")
                except: pass
                try: await cur.execute("ALTER TABLE strife_voice_state ADD COLUMN last_error TEXT NULL")
                except: pass
                try: await cur.execute("CREATE INDEX strife_voice_state_rejoin_idx ON strife_voice_state (bot_name, desired_connected, last_seen_at)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_metrics_status_idx ON strife_metrics (bot_name, updated_at)")
                except: pass
                ...
                try: await cur.execute("ALTER TABLE strife_active_playlists ADD COLUMN channel_id BIGINT DEFAULT NULL")
                except: pass
                ...
                try: await cur.execute("CREATE INDEX strife_queue_lookup_idx ON strife_queue (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_queue_backup_lookup_idx ON strife_queue_backup (guild_id, bot_name, id)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_history_guild_played_idx ON strife_history (guild_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_history_requester_idx ON strife_history (guild_id, requester_id, played_at)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_playback_resume_idx ON strife_playback_state (bot_name, is_playing, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_playlist_bot_idx ON strife_active_playlists (bot_name, guild_id)")
                except: pass
                try: await cur.execute("CREATE INDEX strife_user_playlist_lookup_idx ON strife_user_playlists (user_id, playlist_name)")
                except: pass
                try: await cur.execute("ALTER TABLE strife_playback_state ADD COLUMN bot_name VARCHAR(50) DEFAULT 'strife'")
                except: pass
                try: await cur.execute("ALTER TABLE strife_active_playlists ADD COLUMN bot_name VARCHAR(50) DEFAULT 'strife'")
                except: pass
                try: await cur.execute("ALTER TABLE strife_bot_home_channels ADD COLUMN bot_name VARCHAR(50) DEFAULT 'strife'")
                except: pass
...
@bot.tree.command(name="strife_main_shuffle", description="Randomize the order of the upcoming queue while keeping the currently playing track untouched.")
async def shuffle(interaction: discord.Interaction):
    if not await is_dj(interaction): return
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, video_url, title, requester_id FROM strife_queue WHERE guild_id = %s AND bot_name = 'strife'", (interaction.guild.id,))
